...
def get_env_params() -> Dict:
    ...
    params = dict(
        ...
        KD_REQUIRE_ZONE=bool_env("KD_REQUIRE_ZONE", False),
        KD_ZONE_LOW=float(os.getenv("KD_ZONE_LOW", "40")),
        KD_ZONE_HIGH=float(os.getenv("KD_ZONE_HIGH", "80")),

        # Volume rule: today / avg(N) >= multiplier
        VOLUME_LOOKBACK=int(os.getenv("VOLUME_LOOKBACK", "20")),
        VOLUME_MULTIPLIER=float(os.getenv("VOLUME_MULTIPLIER", "1.5")),

        # Price/MA rules
        ENABLE_RULE_BLACK_CANDLE_LIMIT=bool_env("ENABLE_RULE_BLACK_CANDLE_LIMIT", True),
        BLACK_CANDLE_MAX_DROP=float(os.getenv("BLACK_CANDLE_MAX_DROP", "0.95")),
        ENABLE_RULE_OC_ABOVE_MA20=bool_env("ENABLE_RULE_OC_ABOVE_MA20", True),
        ENABLE_RULE_LAST5_MA10_THRESHOLD=bool_env("ENABLE_RULE_LAST5_MA10_THRESHOLD", True),
        MAX_DAYS_BELOW_MA10_IN_5=int(os.getenv("MAX_DAYS_BELOW_MA10_IN_5", "3")),
        ENABLE_RULE_MA5_GT_MA20=bool_env("ENABLE_RULE_MA5_GT_MA20", True),
    ...
    # 簡單列一下關鍵參數，避免 KeyError
    logger.info(
        "Params loaded: KD=(%d,%d,%d), window=%d, zone=%s[%g~%g], "
        "VOL: N=%d x%.2f, TOP_N=%d, MCAP_MIN=%.0f",
...
def build_universe() -> pd.DataFrame:
    """
    Return DataFrame with columns: code (str), name (str), yahoo (str)
    Only TWSE listed common stocks; excludes ETFs / warrants / etc.
    """
...
def run_once():
    ...
    logger.info("Building universe from TWSE ISIN page...")
    uni = build_universe()
